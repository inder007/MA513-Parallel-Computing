...
int main(int argc, char** argv){
	...
	MPI_Scatter(x, no_of_rows, MPI_INT, b, no_of_rows, MPI_INT, root_process, MPI_COMM_WORLD);
	...
	MPI_Gather(b, no_of_rows, MPI_INT, x, no_of_rows, MPI_INT, root_process, MPI_COMM_WORLD);
	// if(my_id == root_process){
	// 	for(int i=0;i<n;i++){
	// 		printf("%d ", x[i]);;
	// 	}
