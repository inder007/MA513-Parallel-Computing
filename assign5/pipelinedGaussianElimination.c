...
int main(int argc, char **argv){
	...
	for(int i=0;i<n;i++){
		if(i%size == my_id){
			...
			if(size>1){
				MPI_Send(&recvA[i], n-i, MPI_FLOAT, (my_id+1)%size, 0, MPI_COMM_WORLD);
			}
			...
			MPI_Recv(&recvA[i], n-i, MPI_FLOAT, (my_id - 1 + size)%size, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			if(i%size != (my_id+1)%size){
				MPI_Send(&recvA[i], n-i, MPI_FLOAT, (my_id + 1)%size, 0, MPI_COMM_WORLD);
			}
			// MPI_Bcast(&recvA[i], n-i, MPI_FLOAT, i%size, MPI_COMM_WORLD);
			int j;
